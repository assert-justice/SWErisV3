...
public readonly struct SwAnimation(string name, SwFrame[] frames, ErVec2 size, SwAnimationState defaultState)
{
    ...
    // public static bool TryFromAseJson(out SwAnimation animation, PriNode aseData, string name)
    // {
    //     animation = default!;
    //     var meta = aseData.Get("meta");
    //     if(!SwData.TryLoadTexture(out var texture, meta.Get("image"))) return ErEngine.LogWarning("ase animation could not read texture");
    //     var frameTags = meta.Get("frameTags");
    //     var frames = aseData.Get("frames");
    //     PriNode[] tags = [..frameTags.Values];
    ...
    public static bool TryFromData(out SwAnimation animation, PriNode data, ErVec2 frameSize, ErVec2? frameOffset = null, ErVec2? framePadding = null)
    {
        animation = default!;
        if(!data.TryGet("name", out string name)) name = "default";
        if(!SwData.TryLoadTexture(out var texture, data.Get("texture"))) return ErEngine.LogWarning("bad texture");
        SwTileSplitter tileSplitter = new(texture.Size, frameSize, frameOffset, framePadding);
        if(!data.TryGet("first_frame", out int first_frame)) first_frame = 0;
        if(!data.TryGet("last_frame", out int last_frame)) last_frame = tileSplitter.NumFrames - 1;
        if(!data.TryGet("fps", out double fps)) fps = 8;
        if(!data.TryGet("h_flip", out bool h_flip)) h_flip = false;
        if(!data.TryGet("v_flip", out bool v_flip)) v_flip = false;
        if(!data.TryGet("loops", out bool loops)) loops = false;
        if(!data.TryGet("autoplay", out bool autoplay)) autoplay = false;
        if(!SwFrame.TryGetFrames(out var frames, new(texture), frameSize, first_frame, last_frame, frameOffset, framePadding)) return ErEngine.LogWarning(name, " anim failed to get frames");
        SwAnimationState defaultState = new();
